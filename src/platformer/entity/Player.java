...
public class Player extends Entity {
	...
	public void update(ArrayList<Entity> entities, ArrayList<Tile> tiles, ArrayList<String> changes, float xOffset, float yOffset) {
		...
		for (int k = 0; k < tiles.size(); k++) {
			...
			if (tiles.get(k).getX() / 64 == bottomTX - 1 && tiles.get(k).getY() / 64 == bottomTY + 1
					&& tiles.get(k).getZ() == 0)
				onGround = true;
		...
		// keybord
		// jump
		if (Main.isKeyPressed(KeyEvent.VK_SPACE))
			jump();
		// left&right
		if (Main.isKeyPressed(KeyEvent.VK_A))
			velX = -5;
		if (Main.isKeyPressed(KeyEvent.VK_D))
			velX = 5;
		if (!(Main.isKeyPressed(KeyEvent.VK_A) || Main.isKeyPressed(KeyEvent.VK_D)))
			velX = 0;
		// block select
		if (Main.isKeyPressed(KeyEvent.VK_1))
			currentBlock = TileID.Dirt_tile;
		if (Main.isKeyPressed(KeyEvent.VK_2))
			currentBlock = TileID.Dirt_Grass_tile;
		if (Main.isKeyPressed(KeyEvent.VK_3))
			currentBlock = TileID.Greystone_tile;
		if (Main.isKeyPressed(KeyEvent.VK_4))
			currentBlock = TileID.Redstone_tile;
		if (Main.isKeyPressed(KeyEvent.VK_5))
			currentBlock = TileID.Brick_Grey_tile;
		if (Main.isKeyPressed(KeyEvent.VK_6))
			currentBlock = TileID.Brick_Red_tile;
		// mouse
		// destroy block
		if (Main.isMouseButtonPressed(MouseEvent.BUTTON1)) {
			...
			if (now - lastTimeMouse1 >= nsPerMousePress1) {
				...
				// go through the tiles array to see if there is any tiles at
				// mouses tileX and tileY
				for (int i = 0; i < tiles.size(); i++) {
					...
					if (tiles.get(i).getX() / 64 == tx && tiles.get(i).getY() / 64 == ty && tiles.get(i).getZ() == 0) {
						...
						break;
						// if the tile is in the background go through the list
						// again and see if there is one in front of it
					} else if (tiles.get(i).getX() / 64 == tx && tiles.get(i).getY() / 64 == ty
							&& tiles.get(i).getZ() == 1) {
						boolean isForeground = false;
						for (int t = 0; t < tiles.size(); t++) {
							if (tiles.get(t).getX() / 64 == tx && tiles.get(t).getY() / 64 == ty
									&& tiles.get(t).getZ() == 0) {
								isForeground = true;
							}
						}
		...
		if (Main.isMouseButtonPressed(MouseEvent.BUTTON3)) {
			...
			if (now - lastTimeMouse2 >= nsPerMousePress2) {
				...
				System.out.println(tx + " | " + ty + " | " + bottomTY);
				// go through the tiles array and see if the tiles are full
				// where you clicked
				for (int i = 0; i < tiles.size(); i++) {
				...
				if (!isFull1 && !isFull0) {
					...
					saveChange(changes, tx, ty, 1, 'p', currentBlock.toString());
					tiles.add(tempTile);
				...
				// place in the foreground if there is a tile in the background
					...
					saveChange(changes, tx, ty, 0, 'p', currentBlock.toString());
					tiles.add(tempTile);
					System.out.println(tempTile.getX() / 64 + " | " + tempTile.getY() / 64);
				}
	...
	private void saveChange(ArrayList<String> changes, int x, int y, int z, char changeType, String tile){
		String change = Integer.toString(x) + " " + Integer.toString(y) + " " + Integer.toString(z) + " " + changeType + " " + tile;
		if(changes.size() > 0){
